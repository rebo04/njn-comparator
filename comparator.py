...
# ── Apple System Colors ───────────────────────────────────────────────────────
# Changed cell: Apple Orange tint — readable with dark text
FILL_CHANGED = PatternFill("solid", fgColor="FFE5B4")   # warm amber tint
# Added row: Apple System Green
FILL_ADDED   = PatternFill("solid", fgColor="34C759")
# Deleted row: Apple System Red
FILL_REMOVED = PatternFill("solid", fgColor="FF3B30")
...
FONT_STATUS  = Font(name="Century Gothic", size=10,  bold=True, color="FFFFFF")

# Legend — Apple near-black header, clean system-color swatches
FILL_LEGEND_TITLE = PatternFill("solid", fgColor="1C1C1E")   # Apple label/dark
FILL_LEGEND_CHG   = PatternFill("solid", fgColor="FF9500")   # Apple System Orange
FILL_LEGEND_ADD   = PatternFill("solid", fgColor="34C759")   # Apple System Green
FILL_LEGEND_DEL   = PatternFill("solid", fgColor="FF3B30")   # Apple System Red
FILL_LEGEND_SAME  = PatternFill("solid", fgColor="F5F5F7")   # Apple light gray
...
def add_legend(ws, start_row, label_a, label_b, max_col):
    end_col = max_col + 2
    r = start_row + 2  # one blank row gap

    # ── Title bar ────────────────────────────────────────────────────────────
    ws.merge_cells(start_row=r, start_column=1, end_row=r, end_column=end_col)
    title = ws.cell(r, 1, f"  {label_a}  →  {label_b}")
    title.fill = FILL_LEGEND_TITLE
    title.font = Font(name="Century Gothic", size=13, bold=True, color="FFFFFF")
    ...
    # ── Subtitle ─────────────────────────────────────────────────────────────
    ws.merge_cells(start_row=r, start_column=1, end_row=r, end_column=end_col)
    sub = ws.cell(r, 1, f"  Generated by NJN Comparator  ·  {datetime.now().strftime('%Y-%m-%d  %H:%M')}")
    sub.fill = PatternFill("solid", fgColor="2C2C2E")   # Apple secondary label
    sub.font = Font(name="Century Gothic", size=9, color="EBEBF5")  # Apple secondary text
    ...
    # ── Legend items ─────────────────────────────────────────────────────────
    items = [
        (FILL_LEGEND_CHG, "FF9500", "  CHANGED",  "Cell value was modified — hover to see previous value"),
        (FILL_LEGEND_ADD, "34C759", "  ADDED",    "Row is new in this revision"),
        (FILL_LEGEND_DEL, "FF3B30", "  DELETED",  "Row was removed — shown in its original position"),
        (FILL_LEGEND_SAME,"6E6E73", "  UNCHANGED","No difference"),
    ]
    for fill, accent, label, description in items:
        ws.row_dimensions[r].height = 22
        # Swatch column
        swatch = ws.cell(r, 1, label)
        swatch.fill = fill
        swatch.font = Font(name="Century Gothic", size=10, bold=True,
                           color="FFFFFF" if accent != "6E6E73" else "1C1C1E")
        ...
        desc_cell = ws.cell(r, 2, f"  {description}")
        desc_cell.fill = PatternFill("solid", fgColor="F5F5F7")  # Apple light gray
        desc_cell.font = Font(name="Century Gothic", size=10, color="1C1C1E")
...
def compare_and_export(path_a, path_b, out_path, label_a, label_b):
    ...
    if trailing_dels:
        ...
        hdr.fill = PatternFill("solid", fgColor="FF3B30")
